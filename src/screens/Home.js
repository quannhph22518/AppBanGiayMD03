...
const Home = ({ navigation }) => {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const response = await axios.get('http://192.168.10.106:3000/api/brand/');
      ...
      const fetchProducts = async () => {
        try {
          const [popularResponse, newArrivalResponse] = await Promise.all([
            axios.get(`http://192.168.10.106:3000/api/product/tag/${activeCategory}/popular`),
            axios.get(`http://192.168.10.106:3000/api/product/tag/${activeCategory}/new-arrivals`),
          ]);
